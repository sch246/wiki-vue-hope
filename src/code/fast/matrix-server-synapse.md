...
`--no-eff-email --email user@yourdomainname.info`的作用似乎是证书快到期时给你发邮件

```sh
sudo systemctl stop nginx
certbot certonly --rsa-key-size 2048 --standalone --agree-tos --no-eff-email --email user@yourdomainname.info -d yourdomainname.info
