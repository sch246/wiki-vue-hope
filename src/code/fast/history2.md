...
- 注: CSS 实际是 1994 年提出，1996 年发布的，此处捆绑是为了叙事方便。
...
*   **中二描述:** **【理性的修正者】** 它是 OCaml 家族在经历了 ReasonML 的身份认知分裂后，为了征服浏览器而诞生的**最终兵器**。它脱下了晦涩的学院派长袍，换上了类似 JavaScript 的便服，只为潜入敌营。但不要被它的外表欺骗，它的骨子里依然流淌着 ML 家族那**绝对健全（Soundness）**的高贵血液。它鄙视 TypeScript 那漏洞百出的类型系统，用**闪电般的编译速度**和**坚不可摧的类型结界**告诉世人：真正的安全，不需要 `any` 的妥协。
*   **必杀技:** 闪电编译（Lightning fast build）、绝对健全类型（Sound Typing）、零开销互操作。
*   **战力评价:** 防御力（类型安全）远超 TypeScript，编译速度快得像时间被暂停，但因为拒绝了 TS 的庞大生态圈（需绑定），在魔力值上略显孤独。
...
radar-beta
    ...
    %% ReScript: JS运行速度(中), 类型系统极稳(健全), ML抽象高, 编译神速+语法像JS, 生态较小, 略有门槛
    curve a["ReScript"] { 70, 95, 80, 90, 40, 60 }
    max 100
    min 0
    graticule polygon
```
...
*   **中二描述:** **【圣言的速记员】** 如果说 TypeScript 是给 JavaScript 穿上了繁重的黄金圣衣，那么 Civet 就是将这套圣衣炼化为**无形的流体**。它是 CoffeeScript 的亡灵在类型时代的转生。它嫌弃 TypeScript 的咒语过于冗长啰嗦，于是发明了名为“管道”与“缩进”的禁术。它允许法师用原本 1/3 的音节，吟唱出同样强大的类型魔法。它是对“极简”二字的狂热崇拜，将原本笨重的开发体验加速到了肉眼无法捕捉的残影。
*   **必杀技:** 管道操作符 `|>`、类型自动推导简写、JSX 缩写术。
*   **战力评价:** 敏捷度（代码书写效率）突破天际，完全寄生于 TS 生态（魔力值共享），但因为是“方言的方言”，需要施法者拥有极高的悟性才能驾驭。
...
radar-beta
    ...
    %% Civet: 同TS, 同TS, 语法糖极多(高), 写得飞快, 寄生TS生态(高), 需要懂TS+Coffee(较难)
    curve a["Civet"] { 60, 70, 85, 98, 90, 50 }
    max 100
    min 0
    graticule polygon
```
