...
radar-beta
    ...
    curve a["Plankalkül"] { 0, 10, 80, 0, 0, 5 }
    max 100
    min 0
    graticule polygon
```
...
- **完全体修正:** 敏捷度极低（5分），因为在现代 CPU 上手写汇编是极其痛苦且低效的。
- **必杀技:** 直接内存寻址、零开销抽象
...
radar-beta
    axis Perf["【破坏力】极限出力"], Stab["【防御力】绝对境界"], Abst["【精神力】构筑术式"]
    axis DevEff["【敏捷度】神之手"], Eco["【魔力值】无限书库"], Ease["【同调率】凡人亲和"]
    curve a["Assembly"] { 100, 5, 5, 5, 20, 1 }
    max 100
    min 0
    graticule polygon
```
...
- **中二描述:** **【凡人的通用语】** 它是普罗米修斯盗来的第二把火，将编程的权柄从科学家手中夺下，分发给每一个拥有微型计算机的孩子。它简单、直接、毫无门槛，虽然被精英们鄙视（GOTO 面条代码），但它不仅征服了世界，还成为了比尔·盖茨等无数英雄的启蒙导师。
- **必杀技:** GOTO 跳转、解释执行、INPUT/PRINT
- **战力评价:** 同调率满分！虽然其他属性全是渣，但依靠庞大的群众基础（生态），它赢了。
...
radar-beta
    ...
    curve a["BASIC"] { 10, 10, 20, 80, 90, 100 }
    max 100
    min 0
    graticule polygon
```
...
- **完全体修正:** 防御力极低（5分），因为缓冲区溢出依然是现代软件安全的最大噩梦。
- **必杀技:** 指针算术、内联汇编
...
- **必杀技:** 组合子逻辑
- **战力评价:** 思想过于超前，肉体无法承载（当时的硬件无法高效运行），只能活在论文里。
...
- **完全体修正:** 精神力极高（90分，模板元编程图灵完备），敏捷度极低（20分，编译慢、心智负担重）。
- **必杀技:** 模板元编程、RAII、移动语义
...
radar-beta
    axis Perf["【破坏力】极限出力"], Stab["【防御力】绝对境界"], Abst["【精神力】构筑术式"]
    axis DevEff["【敏捷度】神之手"], Eco["【魔力值】无限书库"], Ease["【同调率】凡人亲和"]
    curve a["C++ (Modern)"] { 97, 50, 90, 20, 95, 15 }
    max 100
    min 0
    graticule polygon
```
...
radar-beta
    ...
    curve a["Perl"] { 40, 20, 50, 95, 80, 30 }
    max 100
    min 0
    graticule polygon
```
...
radar-beta
    ...
    curve a["Haskell"] { 80, 100, 100, 60, 40, 5 }
    max 100
    min 0
    graticule polygon
```
...
- **完全体修正:** 精神力上调至 90（极高维度的抽象），同调率极低（2分，甚至是负分）。
- **必杀技:** 隐式迭代、函数秩 (Rank)
...
### **1991 - Python (Modern / 3.12+)**

- **中二描述:** **【吞噬世界的尘世巨蟒】** 它已经不再是当年的那个脚本小子。它吞噬了 AI、数据科学、Web 和自动化运维。虽然它的肉体（解释器）依然孱弱，但它驾驭着无数由 C/C++ 打造的神器（NumPy, PyTorch）。它是新时代的通用语，只要你会说话，你就能施法。
- **完全体修正:** 破坏力下调至 10（纯 Python 依然慢），敏捷度与魔力值（生态）依然是满分。
...
radar-beta
    axis Perf["【破坏力】极限出力"], Stab["【防御力】绝对境界"], Abst["【精神力】构筑术式"]
    axis DevEff["【敏捷度】神之手"], Eco["【魔力值】无限书库"], Ease["【同调率】凡人亲和"]
    curve a["Python"] { 10, 40, 80, 100, 100, 95 }
    max 100
    min 0
    graticule polygon
```
...
- **完全体修正:** 破坏力修正为 65（虚拟机开销），精神力修正为 50（虽然有了新特性，但历史包袱重）。
...
radar-beta
    axis Perf["【破坏力】极限出力"], Stab["【防御力】绝对境界"], Abst["【精神力】构筑术式"]
    axis DevEff["【敏捷度】神之手"], Eco["【魔力值】无限书库"], Ease["【同调率】凡人亲和"]
    curve a["Java (Modern)"] { 65, 75, 50, 65, 100, 60 }
    max 100
    min 0
    graticule polygon
```
...
- **战力评价:** 敏捷度极高，魔力值全领域覆盖，但防御力（弱类型）极差，容易走火入魔。
...
radar-beta
    ...
    curve a["JavaScript"] { 60, 10, 50, 90, 100, 85 }
    max 100
    min 0
    graticule polygon
```
...
- **完全体修正:** 敏捷度修正为 20（写 Rust 是与编译器搏斗的过程，极度消耗脑力），防御力满分。
...
radar-beta
    axis Perf["【破坏力】极限出力"], Stab["【防御力】绝对境界"], Abst["【精神力】构筑术式"]
    axis DevEff["【敏捷度】神之手"], Eco["【魔力值】无限书库"], Ease["【同调率】凡人亲和"]
    curve a["Rust (Stable)"] { 96, 100, 85, 20, 70, 10 }
    max 100
    min 0
    graticule polygon
```
...
- **完全体修正:** 敏捷度 95（极其优秀的 IDE 支持），防御力 70（编译期拦截，但运行时依然是 JS）。
- **必杀技:** 结构化类型系统、类型体操
...
radar-beta
    ...
    curve a["Solidity"] { 10, 10, 30, 40, 80, 40 }
    max 100
    min 0
    graticule polygon
```
...
- **中二描述:** **【AI 的终极魔神】** 它是 Python 的超进化体，拥有 Python 的皮囊和 C++ 的内脏。它试图填平研究与部署之间的鸿沟，让 AI 的火焰燃烧得更猛烈。虽然它还年轻，生态尚未丰满，但它展现出的理论战力（比 Python 快 35000 倍）足以让旧神颤抖。
- **完全体修正:** 破坏力 100（理论值），魔力值 10（还是婴儿）。
...
radar-beta
    axis Perf["【破坏力】极限出力"], Stab["【防御力】绝对境界"], Abst["【精神力】构筑术式"]
    axis DevEff["【敏捷度】神之手"], Eco["【魔力值】无限书库"], Ease["【同调率】凡人亲和"]
    curve a["Mojo"] { 100, 70, 80, 95, 10, 90 }
    max 100
    min 0
    graticule polygon
```
