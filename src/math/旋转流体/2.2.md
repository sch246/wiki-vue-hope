...
![20241107184128](https://s2.loli.net/2024/11/07/QjlMRSB3sVPb9DA.png)
...
含有 $\delta_{ij}$ 的项表示在张量对角线上，表示各向同性的属性，例如压力

其中 $-p\delta_{ij}$ 为静水压力，其中标量 $p$ 表示平均压力大小，加负号是因为需要保证压力向内时 $p$ 为正
...
第一项为体积不变时的应力，第二项为体积变化导致的应力

张量对角线的值表示拉伸作用，这里的和（迹）正比于体积变化率，也就是速度散度
...
$$
\mathbf{f} = \frac{\partial \tau_{ij}}{\partial x_j}e_i\tag{爱因斯坦求和约定}
$$
