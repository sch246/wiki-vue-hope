...
它对基的作用可以分解为 $\cos\theta$ 和 $i\sin\theta$，分别对应当前方向的放缩和当前方向旋转90度方向上的放缩，而总影响是它们的组合，也就是旋转
...
就像任意复数分解到实数轴和虚数轴一样，我们可以将任意四元数分解为 $q = q_0+hq_h$，其中 $h$ 为左乘的四元数 $q$ 在 $ijk$ 空间上分量方向的单位四元数
...
于是可独立计算放缩和旋转，互不影响，也证明了四元数乘法模长守恒
...
在这个投影下，离原点的距离意味着按右手定则旋转的角度。原点表示不变，单位球面上的点对应旋转90度，到无穷远点是180度
