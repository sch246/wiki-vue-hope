...
在这个投影下，离原点的距离与旋转程度正相关（对应四元数左乘的作用）。原点表示不变，单位球面上的点对应旋转90度，到无穷远点是180度，且关于原点对称的点互为逆/共轭（不是相反数，因为实部不变）
...
这是唯一不遵循交换律的地方，导致了右乘时 $\{1, h\}$ 旋转相同而虚空间旋转反向

于是当我们右乘一个逆时，这产生了另一个相反的双旋转，在 $\{1, h\}$ 上反向而刚好抵消，在虚空间上的旋转负负得正，二倍了原本的旋转
